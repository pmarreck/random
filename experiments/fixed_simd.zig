//! Opt-in experiment only. No production sampler imports this module.
...
fn u(n: u64) U { return @splat(n); }
fn i(n: i64) I { return @splat(n); }
fn sh(n: u6) @Vector(4, u6) { return @splat(n); }
fn splat(x: fx.Fixed) V { return .{ .m = i(x.m), .e = i(x.e) }; }
fn pack(xs: Batch) V {
	var out: V = undefined;
	inline for (0..4) |lane| { out.m[lane] = xs[lane].m; out.e[lane] = xs[lane].e; }
	return out;
}
fn unpack(x: V) Batch {
...
fn magnitude(m: I) U { return @bitCast(@select(i64, m < i(0), -%m, m)); }
...
fn mul(a: V, b: V) V {
	...
	return .{ .m = signed(mag, (a.m < i(0)) != (b.m < i(0))),
		.e = @select(i64, mag == u(0), i(0), a.e + b.e + @select(i64, high, i(1), i(0))) };
}
...
fn add(a: V, b: V) V {
	...
	var out = norm(@select(i64, same, half_sum, x.m +% shifted),
		x.e + @select(i64, same, i(1), i(0)));
	out = choose(d >= i(63), x, out);
	out = choose(b.m == i(0), a, out);
	return choose(a.m == i(0), b, out);
}

pub fn mul4(a: Batch, b: Batch) Batch { return unpack(mul(pack(a), pack(b))); }
pub fn add4(a: Batch, b: Batch) Batch { return unpack(add(pack(a), pack(b))); }
