...
-- division loop itself. `jit.off(M.div, true)` (recursive: covers M.norm,
-- which M.div calls) eliminates it with zero mismatches across 20000
-- warmup iterations in testing, at the cost of losing JIT compilation for
-- this one function. No evidence of the same bug in M.mul was found in
-- targeted testing (200000 negative-operand calls cross-checked against
-- jit.off), but that is not proof of absence -- M.mul was not built to
-- withstand adversarial fuzzing for this specific failure mode, only
-- checked for it after the fact. This is a LATENT LANGUAGE-RUNTIME BUG,
-- not a defect in this file's logic; given this file's entire purpose is
-- guaranteeing bit-exact results, a JIT compiler that silently returns a
-- different answer for the same inputs after enough prior calls is
-- disqualifying for the affected function until fixed upstream or
-- otherwise worked around. See tests/fixed_test.lua's "JIT miscompilation
-- regression (warmed)" check, which re-triggers the warmup pattern and
-- would fail loudly if this protection is ever removed.
jit.off(M.div, true)
